...
export type DeprecateFeatureOptions = {
  featureId: string;
  reason: string;
  actorId?: string;
...
export type DeprecateFeatureResult = {
  featureId: string;
  eventId: string;
  file: string;
};

export async function deprecateFeature(options: DeprecateFeatureOptions): Promise<DeprecateFeatureResult> {
  ...
  if (validation.errors.length > 0) {
    ...
    throw new Error(`Cannot deprecate feature because the current event stream is invalid:\n${details}`);
  }
