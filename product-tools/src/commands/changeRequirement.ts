...
export type ChangeRequirementResult = {
  requirementId: string;
  eventId: string;
  file: string;
};

export async function changeRequirement(options: ChangeRequirementOptions): Promise<ChangeRequirementResult> {
  ...
  if (validation.errors.length > 0) {
    ...
    throw new Error(`Cannot change requirement because the current event stream is invalid:\n${details}`);
  }

  const state = replayEvents(validation.events);
  const requirement = state.requirements.get(options.requirementId);
  if (!requirement) {
    throw new Error(`Requirement '${options.requirementId}' does not exist`);
  }
