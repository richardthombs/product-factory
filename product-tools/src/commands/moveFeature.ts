...
export async function moveFeature(options: MoveFeatureOptions): Promise<MoveFeatureResult> {
  ...
  if (validation.errors.length > 0) {
    ...
    throw new Error(`Cannot move feature because the current event stream is invalid:\n${details}`);
  }
  ...
  if (feature.capabilityId === capability.id) {
    throw new Error(`Feature '${feature.id}' is already under capability '${capability.id}'`);
  }
