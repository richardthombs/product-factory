...
import { DEFAULT_FEATURE_STATUS, FEATURE_STATUSES, type FeatureStatus } from "../domain/status.js";
...
export type SetFeatureStatusResult = {
  featureId: string;
  status: FeatureStatus;
  eventId: string;
  file: string;
...
export async function setFeatureStatus(options: SetFeatureStatusOptions): Promise<SetFeatureStatusResult> {
  ...
  if (validation.errors.length > 0) {
    ...
    throw new Error(`Cannot set feature status because the current event stream is invalid:\n${details}`);
  }
  ...
  if (!FEATURE_STATUSES.includes(options.status)) {
    throw new Error(`Invalid feature status '${options.status}'. Expected one of: ${FEATURE_STATUSES.join(", ")}`);
  }

  if (feature.status === options.status && (options.reason ?? "") === (feature.statusReason ?? "")) {
    throw new Error(`Feature '${feature.id}' already has status '${options.status}'`);
  }
  ...
  try {
    ...
    return {
      ...
      status: options.status ?? DEFAULT_FEATURE_STATUS,
      eventId: event.id,
      file,
    };
  } catch (error) {
