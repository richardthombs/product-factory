...
import { createTest } from "../commands/createTest.js";

async function main(): Promise<void> {
  const args = parseArgs(process.argv.slice(2));
  const result = await createTest({
    acceptanceCriterionIds: args["acceptance-criterion"] ?? [],
    filePath: requiredArg(args, "file"),
    testName: requiredArg(args, "test-name"),
  ...
  console.log("Created test artifacts for acceptance criteria.");
...
function parseArgs(argv: string[]): Record<string, string[]> {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!token.startsWith("--")) {
      throw new Error(`Unexpected argument '${token}'. Expected flags like --file value`);
    }
...
function singleArg(args: Record<string, string[]>, key: string): string | undefined {
  ...
  if (values.length > 1 && key !== "acceptance-criterion") {
