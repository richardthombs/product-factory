...
import { changeRequirement } from "../commands/changeRequirement.js";

async function main(): Promise<void> {
  const args = parseArgs(process.argv.slice(2));
  const result = await changeRequirement({
    requirementId: requiredArg(args, "requirement"),
  ...
  console.log("Created proposed requirement change event.");
...
function parseArgs(argv: string[]): Record<string, string[]> {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!token.startsWith("--")) {
      throw new Error(`Unexpected argument '${token}'. Expected flags like --requirement value`);
    }
