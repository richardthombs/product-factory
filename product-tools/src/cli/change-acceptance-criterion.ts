...
import { changeAcceptanceCriterion } from "../commands/changeAcceptanceCriterion.js";

async function main(): Promise<void> {
  const args = parseArgs(process.argv.slice(2));
  const result = await changeAcceptanceCriterion({
    acceptanceCriterionId: requiredArg(args, "acceptance-criterion"),
  ...
  console.log("Created proposed acceptance criterion change event.");
  console.log(`- acceptance criterion: ${result.acceptanceCriterionId}`);
...
function parseArgs(argv: string[]): Record<string, string[]> {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!token.startsWith("--")) {
      throw new Error(`Unexpected argument '${token}'. Expected flags like --acceptance-criterion value`);
    }
