...
import { setFeatureStatus } from "../commands/setFeatureStatus.js";

async function main(): Promise<void> {
  const args = parseArgs(process.argv.slice(2));
  const result = await setFeatureStatus({
    featureId: requiredArg(args, "feature"),
    status: requiredArg(args, "status") as Parameters<typeof setFeatureStatus>[0]["status"],
    reason: singleArg(args, "reason"),
  ...
  console.log("Created proposed feature status change event.");
...
function parseArgs(argv: string[]): Record<string, string[]> {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!token.startsWith("--")) {
      throw new Error(`Unexpected argument '${token}'. Expected flags like --feature value`);
    }
