...
import { setCapabilityStatus } from "../commands/setCapabilityStatus.js";

async function main(): Promise<void> {
  const args = parseArgs(process.argv.slice(2));
  const result = await setCapabilityStatus({
    capabilityId: requiredArg(args, "capability"),
    status: requiredArg(args, "status") as Parameters<typeof setCapabilityStatus>[0]["status"],
  ...
  console.log("Created proposed capability status change event.");
...
function parseArgs(argv: string[]): Record<string, string[]> {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!token.startsWith("--")) {
      throw new Error(`Unexpected argument '${token}'. Expected flags like --capability value`);
    }
