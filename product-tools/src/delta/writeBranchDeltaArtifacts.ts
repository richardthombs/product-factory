...
export const DEFAULT_BRANCH_DELTA_YAML_PATH = path.join("indexes", "branch-delta.yaml");
...
export async function writeBranchDeltaArtifacts(options: WriteBranchDeltaArtifactsOptions): Promise<WriteBranchDeltaArtifactsResult> {
  ...
  const modelRoot = path.resolve(cwd, options.modelRoot ?? "product-model");
  const report = await branchDelta(options);
  const markdownPath = path.join(modelRoot, DEFAULT_BRANCH_DELTA_MARKDOWN_PATH);
  const yamlPath = path.join(modelRoot, DEFAULT_BRANCH_DELTA_YAML_PATH);
